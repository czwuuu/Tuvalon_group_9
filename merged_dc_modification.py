...
# 基类
'''
建议：每个角色的策略尽量都用kind_level(最好不用suspicious_level)。trusted_good集合只在游戏开始时更新，之后不改变（因为无法确定一个选手真的是蓝方阵营）；trusted_evil集合可以根据投票结果更新。判断某个选手的阵营时按照trusted_good > sorted(kind_level) > trusted_evil进行。
    组队策略建议蓝方始终只选最可能是蓝方的选手；红方选择自己（如果是第四轮再选择1个最可能是红方的选手）和剩下中最可能是蓝方的选手。
'''
class RoleStrategy:
